...
function getActiveDropdown() {
    return APP_STATE.token ? document.getElementById('user-menu-user') : document.getElementById('user-menu-guest');
}
    ...
    switch (hash) {
        ...
        case '#report':
            const rData = sessionStorage.getItem('viewReportData'),
                iData = sessionStorage.getItem('viewInputsData');
        ...
        case '#my-reports':
            if (APP_STATE.token) {
                if (pageSections.myReports) pageSections.myReports.style.display = 'block';
                loadUserReports();
            } else {
...
async function handleUserRegister(e) {
    ...
    apiCall('register', {
        ...
        if (d.ok) handleUserLogin(e);
            ...
            showErr('register-error', d.error);
            navigate();
        }
    });
}
...
async function apiCall(ept, body, cb, auth = false) {
    try {
        ...
        showErr(ept.includes('login') ? 'login-error' : ept.includes('register') ? 'register-error' : 'form-error', 'Network error.');
        navigate();
    }
}
...
function loadUserReports() {
    ...
    reportsEl.innerHTML = "<h3>My Past Reports</h3><div>Loading reports...</div>";
    ...
    fetch(API_URL + "my-reports", {
        ...
        .then(data => {
            if (data.ok && data.reports && data.reports.length) {
                reportsEl.innerHTML = "<h3>My Past Reports</h3>" + data.reports
                    .map((r, idx) =>
                        `<div class="past-report-item">
              <span>${r.firstName} - ${r.dob}</span>
              <button onclick="viewReport(${idx})">View Report</button>
            </div>`
                    ).join('');
                window.LOADED_REPORTS = data.reports;
            } else {
                reportsEl.innerHTML = "<h3>My Past Reports</h3><div>No reports found.</div>";
            }
        })
        .catch(() => {
            reportsEl.innerHTML = "<h3>My Past Reports</h3><div>Error loading reports.</div>";
        });
}
...
// --- YOUR ORIGINAL REPORT RENDERING FUNCTIONS ---
function renderTeaserReport(r) {
    pageSections.report.innerHTML = `<div class="p-8 bg-gray-800 rounded-lg shadow-lg text-center">
        <h2 class="text-3xl font-bold text-white mb-4">Your Basic Numbers</h2>
        <div class="flex justify-center gap-8 mb-6">
            <div class="result-card p-4 bg-purple-600 rounded-lg">
                <h3 class="text-xl font-semibold text-white">Driver Number</h3>
                <p class="text-4xl font-bold text-white">${r.driverNumber}</p>
            </div>
            <div class="result-card p-4 bg-purple-600 rounded-lg">
                <h3 class="text-xl font-semibold text-white">Conductor Number</h3>
                <p class="text-4xl font-bold text-white">${r.conductorNumber}</p>
            </div>
        </div>
        <p class="text-gray-300 mb-4">This is just a glimpse. A full report will reveal your path, strengths, and challenges.</p>
        <a href="#register" class="inline-block bg-green-500 text-white font-bold py-2 px-4 rounded hover:bg-green-600 transition duration-300">Sign Up for Full Report</a>
    </div>`;
}

function renderFullReport(r, i) {
    const formattedName = `${i.firstName}${i.middleName ? ' ' + i.middleName : ''} ${i.lastName}`;

    let personalYearHTML = '';
    if (r.personalYear) {
        personalYearHTML = `
            <div class="bg-gray-700 p-4 rounded-lg">
                <h3 class="text-lg font-bold text-purple-400 mb-2">Personal Year Analysis</h3>
                <p><strong>Personal Year Number:</strong> ${r.personalYear.personalYearNumber}</p>
                <p><strong>Interpretation:</strong> ${r.personalYear.interpretation}</p>
            </div>`;
    }

    let monthlyPredictionsHTML = '';
    if (r.monthlyPredictions) {
        monthlyPredictionsHTML = '<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">';
        r.monthlyPredictions.forEach(m => {
            monthlyPredictionsHTML += `
                <div class="bg-gray-700 p-4 rounded-lg">
                    <h4 class="font-bold text-purple-400">${m.month}</h4>
                    <p><strong>Personal Month Number:</strong> ${m.personalMonthNumber}</p>
                    <p>${m.interpretation}</p>
                </div>`;
        });
        monthlyPredictionsHTML += '</div>';
    }

    pageSections.report.innerHTML = `
        <div class="max-w-4xl mx-auto p-6 bg-gray-800 text-white rounded-lg shadow-xl">
            <div class="text-center mb-8">
                <h2 class="text-4xl font-bold text-purple-400">Numerology Report</h2>
                <p class="text-xl text-gray-300">For: ${formattedName}</p>
                <p class="text-lg text-gray-400">Date of Birth: ${i.dob}</p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div class="result-card p-6 bg-purple-700 rounded-lg text-center">
                    <h3 class="text-2xl font-bold">Driver Number</h3>
                    <p class="text-5xl font-extrabold">${r.driverNumber}</p>
                </div>
                <div class="result-card p-6 bg-purple-700 rounded-lg text-center">
                    <h3 class="text-2xl font-bold">Conductor Number</h3>
                    <p class="text-5xl font-extrabold">${r.conductorNumber}</p>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div>
                    <h3 class="text-2xl font-bold text-purple-400 mb-4">Monthly Predictions</h3>
                    ${monthlyPredictionsHTML}
                </div>
            </div>
        </div>`;
